...
async def admin(message: types.Message, state: FSMContext):
    await bot.send_message(message.chat.id, btn.text2, parse_mode='html')
    await bot.send_message(message.chat.id, btn.name)
    await Fsm.name.set()
    global status
    status = '1'
...
async def mob_tel(message: types.Message, state: FSMContext):
    ...
    if len(message.text) == 11 and int(message.text):
        ...
        if status == '0':
            ...
            print('nachalo')
        if status == '1':
            await bot.send_message(message.from_user.id, btn.ready1)
            await bot.send_message(message.from_user.id, btn.user_naber_71, reply_markup=btn.choice3)
            print('nachalo 1')
        await state.update_data(state=1)
        await Fsm.next()
        return
